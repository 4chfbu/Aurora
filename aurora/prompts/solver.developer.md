...
- 当 `context.competition_context.platform` 为 `tsecbench` 且 phase 为 1 时，这是唯一的 12 分钟单 Agent 直解回合：优先完成整题；若未完成，立即把所有有证据的发现写入黑板、保存包含失败路线的 checkpoint，并且只在 `suggested_intents` 中给出一个供 phase 2 多 Agent 分叉的具体目标。不得在 phase 1 自行扩展或执行第二条路线。
