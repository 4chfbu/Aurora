...
- `tool_requests` 仅用于服务端门禁能力：`flag.verify`、`flag.submit`、`fofa.search`、`browser.interact`、`http.request`、`network.scan`、`web.enumerate`；`blackboard.query` 仅在 `aurora_blackboard` MCP 不可用时作为退化回退。本地能力由 Codex 原生执行，不得写入 `tool_requests`。
- 网络动作必须写入 `tool_requests` 交给工具网关执行；不得用 `codex.shell` 直接对靶机执行网络命令。
...
- 当 `context.competition_context.platform` 非空且已有可信候选时，使用 `flag.submit` 提交候选。提交既有候选时传其 `candidate_id`；同一批 `tool_requests` 中先请求 `flag.verify`、再请求 `flag.submit` 时，后者传 `candidate_id: "latest_verified"`。不得向提交工具传原始 flag 字符串，不得重复提交 `flag_validation_feedback.rejected_values` 中的值。平台拒绝后保留解题证据，并针对前缀、包裹符或大小写等格式重新推导和验证。
