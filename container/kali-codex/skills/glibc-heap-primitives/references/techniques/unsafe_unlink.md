...
- README summary: Exploiting free on a corrupted chunk to get arbitrary write.
- Core effect: arbitrary write by freeing a corrupted chunk that passes unlink checks.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- controllable `fd` and `bk` plus a free path on the corrupted chunk.
...
- The family stays the same: corrupt a chunk so unlink-like allocator writes become useful.
- The practical constraints evolve with allocator checks, so later versions should be treated as adapted demonstrations rather than evidence that old unlink assumptions still hold verbatim.
- When using this page during solving, focus on which consistency checks must be satisfied on the target runtime.
