...
- README summary: Exploiting the overwrite of a freed chunk on large bin freelist to write a large value into arbitrary address
- Core effect: write a large value into a chosen address via large-bin metadata.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- a chunk that really reaches largebin and a writable target.
...
- The largebin write family stays conceptually stable across the retained versions.
- The main cross-version question is whether the target chunk really reaches largebin under the current allocator and surrounding caches.
- Later variants primarily adjust bookkeeping and environment text rather than changing the core write effect.
