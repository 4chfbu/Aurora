...
- README summary: Exploiting the overwrite of a freed chunk on small bin freelist to trick malloc into returning an arbitrary pointer and write a large value into arbitraty address with the help of calloc.
- Core effect: combine smallbin metadata abuse with tcache stashing to gain chosen return and allocator write effects.
- README applicability: `> 2.25`
- Solver window note: `2.27 -> 2.40`

## When To Consider It

- smallbin corruption in tcache era plus `calloc`-driven stash behavior.
...
- The family remains a hybrid of smallbin metadata abuse and tcache stash behavior.
- Cross-version differences mostly track allocator hardening while the same hybrid idea is preserved.
- The critical question is still whether the target chunk reaches the right smallbin and then triggers the stash path.

## Representative Source Code

- Representative version: `glibc 2.40`
