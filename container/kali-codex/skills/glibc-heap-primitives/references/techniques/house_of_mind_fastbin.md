...
- README summary: Exploiting a single byte overwrite with arena handling to write a large value (heap pointer) to an arbitrary address
- Core effect: use arena handling and limited overwrite to place a heap pointer at an arbitrary address.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- single-byte or partial overwrite where arena-side effects matter more than plain freelist poisoning.
...
- 2.23 -> 2.24 uses the older `malloc_state` layout, so the fake arena write target sits at `+0x28` and `system_mem` is patched at offset `0x880`.
- 2.27 updates those allocator offsets to `+0x30` and `0x888`, and it explicitly fills tcache first so the fastbin path is still exercised.
- 2.31 -> 2.41 keeps the 2.27-era allocator layout assumptions; later variants mostly update narration rather than the primitive.
