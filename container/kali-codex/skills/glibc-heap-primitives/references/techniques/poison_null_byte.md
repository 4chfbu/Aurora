...
- README summary: Exploiting a single null byte overflow.
- Core effect: use a single null byte to alter chunk state and drive overlap or consolidation.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- off-by-one null overwrite into next chunk metadata.
...
- The family remains a null-byte metadata corruption technique across the corpus.
- The later retained examples keep the same basic idea but need to be read alongside modern allocator constraints such as tcache and safe-linking-era follow-on primitives.
- The null byte itself is rarely the endgame; the important cross-version difference is what allocator state it can still create.
