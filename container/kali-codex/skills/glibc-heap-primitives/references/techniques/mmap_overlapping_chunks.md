...
- README summary: Exploit an in use mmap chunk in order to make a new allocation overlap with a current mmap chunk
- Core effect: overlap involving `mmap` chunks.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- large allocations that bypass the normal heap arena.
...
- The overlap idea stays stable across the retained versions.
- The main solver difference is whether the target allocation actually goes through the `mmap` path on the chosen runtime.
- Cross-version differences are mostly allocator-environment details, not a new primitive.
