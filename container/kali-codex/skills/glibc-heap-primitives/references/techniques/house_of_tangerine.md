...
- Family: `Wilderness, Arena, And Hybrid Paths`
- README summary: Exploiting the Top Chunk (Wilderness) in order to trick malloc into returning a completely arbitrary pointer by abusing the tcache freelist
- Core effect: arbitrary return through top-chunk and tcache interaction.
- README applicability: `>= 2.26`
- Solver window note: `2.27 -> 2.41`

## When To Consider It

- modern wilderness steering where tcache is part of the finish.
...
- 2.27 -> 2.31 keeps the same modernized house-of-orange style chain: wilderness steering plus a tcache-based finish, without safe-linking pointer mangling.
- 2.32 -> 2.41 updates the final tcache corruption to safe-linking form, encoding the forged pointer with `target ^ (vuln_tcache >> 12)` instead of writing a raw pointer.
- The tested-version comments are also updated to newer glibc releases, but the main conceptual shift is the safe-linking-aware poisoning step.
