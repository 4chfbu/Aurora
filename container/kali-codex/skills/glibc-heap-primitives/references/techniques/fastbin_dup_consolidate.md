...
- README summary: Tricking malloc into returning an already-allocated heap pointer by putting a pointer on both fastbin freelist and the top chunk.
- Core effect: reclaim a chunk through fastbin plus top-chunk interaction.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- fastbin corruption that survives consolidation.
...
- The primitive stays stable across versions: use fastbin duplication together with top-chunk interaction.
- Tcache-era variants require more heap-state setup so the fastbin path remains visible.
- The later examples mostly differ in allocator-environment setup, not in the core idea.
