...
- README summary: Tricking malloc into return a pointer to arbitrary memory by manipulating the tcache management struct by UAF in a free'd tcache chunk.
- Core effect: chosen return via corruption of tcache management state.
- README applicability: `2.31 - 2.33`
- Solver window note: `2.31 -> 2.33`

## When To Consider It

- UAF on freed tcache chunks in the narrow version window where this pattern applies.
...
- No material source differences appear across `2.31 -> 2.33` in the retained corpus.
- Treat it as one narrow-window tcache-management-structure corruption technique.

## Representative Source Code

- Representative version: `glibc 2.31`
