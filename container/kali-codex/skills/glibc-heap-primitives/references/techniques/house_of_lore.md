...
- README summary: Tricking malloc into returning a nearly-arbitrary pointer by abusing the smallbin freelist.
- Core effect: chosen return through smallbin freelist abuse.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- controlled smallbin links and a path that keeps the chunk out of tcache.
...
- 2.23 -> 2.24 is the classic smallbin fake-chunk-on-stack technique without tcache workarounds.
- 2.27 adds explicit tcache handling: dummy allocations are used to fill and later drain tcache, and the fake stack structures are expanded into a fake freelist so the smallbin-to-tcache transition does not crash.
- 2.31 -> 2.34 keeps the same tcache-aware structure as 2.27; the main changes are version retargeting and comments.
- 2.35 -> 2.41 again keeps the same exploitation shape, with only minor retuning and updated environment text.
