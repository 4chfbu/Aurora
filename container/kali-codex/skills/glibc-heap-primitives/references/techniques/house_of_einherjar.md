...
- README summary: Exploiting a single null byte overflow to trick malloc into returning a controlled pointer
- Core effect: clear `prev_inuse`, consolidate backward into a fake chunk, then build overlap or chosen return.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- null-byte overflow plus a known heap address.
...
- 2.23 -> 2.24 is the classic form: a fake chunk, usually on the stack, plus a null-byte overflow that clears `prev_inuse` and consolidates backward.
- 2.27 adapts the fake chunk layout to post-2.26 consistency checks. The fake chunk must carry real-looking `prev_size` and `size`, and the target chunk is enlarged so the free path avoids tcache interference.
- 2.31 is effectively a second-generation rewrite. It fills tcache explicitly, places the fake chunk in the heap, and turns the overlap into a tcache poisoning setup rather than stopping at backward consolidation.
- 2.32 -> 2.41 keeps the 2.31 structure but adds safe-linking constraints: aligned target selection, encoded poisoned pointer values, and an extra padding step before poisoning.
