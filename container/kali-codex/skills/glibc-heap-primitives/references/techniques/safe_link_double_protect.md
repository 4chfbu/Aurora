...
- README summary: Leakless bypass for PROTECT_PTR by protecting a pointer twice, allowing for arbitrary pointer linking in t-cache
- Core effect: bypass `PROTECT_PTR` by protecting a pointer twice.
- README applicability: `>= 2.32`
- Solver window note: `2.32 -> 2.41`

## When To Consider It

- a path that lets you re-encode controlled pointers rather than only decode them.
...
- All retained variants are safe-linking-era demonstrations.
- The core idea stays stable: re-protect a pointer twice so the allocator accepts a controlled link.
- This is a modern bypass pattern rather than a family that materially changes across the retained versions.
