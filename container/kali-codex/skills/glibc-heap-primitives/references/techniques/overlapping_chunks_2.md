...
- README summary: Exploit the overwrite of an in use chunk size in order to make a new allocation overlap with an existing chunk
- Core effect: overlap through in-use chunk size corruption.
- README applicability: `< 2.29`
- Solver window note: `2.23 -> 2.24`

## When To Consider It

- historical targets where a live chunk header is directly reachable.
...
- This is a narrow historical variant that targets in-use chunk size corruption.
- Only 2.23 and 2.24 are retained, and the technique should be treated as historical-only.
- There is no modern variant in the corpus.
