...
- README summary: Exploiting the overwrite of a freed chunk in the fastbin to write a large value into an arbitrary address.
- Core effect: turn fastbin corruption into a large write via tcache refill behavior.
- README applicability: `> 2.25`
- Solver window note: `2.27 -> 2.41`

## When To Consider It

- fastbin corruption on a size that later refills tcache.
...
- This is fundamentally a tcache-era technique, so all retained variants start at 2.27.
- The later variants keep the same fastbin-to-tcache refill idea, with changes mainly driven by newer allocator hardening.
- When the target is 2.32+, reassess any raw pointer assumptions against safe-linking.
