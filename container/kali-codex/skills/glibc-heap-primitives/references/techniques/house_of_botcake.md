...
- README summary: Bypass double free restriction on tcache. Make `tcache_dup` great again.
- Core effect: bypass tcache double-free restrictions and recover a chosen-return primitive.
- README applicability: `> 2.25`
- Solver window note: `2.27 -> 2.41`

## When To Consider It

- tcache-era double free where naive duplication is blocked.
...
- 2.27 -> 2.31 keeps the original botcake idea: create overlap through consolidation, then use the overlapped chunk to write a raw tcache target pointer.
- 2.32 -> 2.41 keeps the same double-free and consolidation setup, but the poisoning step is rewritten for safe-linking: the overlapped write stores an encoded `next` pointer using `((long)a >> 12) ^ target`.
- The newer variant also treats overlap as an explicit intermediate primitive instead of jumping directly to the final chosen return.
