...
- README summary: Tricking malloc into returning a completely arbitrary pointer by abusing the tcache freelist. (requires heap leak on and after 2.32)
- Core effect: chosen return through tcache freelist corruption.
- README applicability: `> 2.25`
- Solver window note: `2.27 -> 2.41`

## When To Consider It

- UAF or stale edit on a freed tcache chunk.
- from `2.32+`, the corpus explicitly requires a heap leak and alignment handling.
...
- 2.27 -> 2.31 is the classic raw-pointer tcache poisoning family.
- 2.32+ adds safe-linking and alignment constraints, so a heap leak or bypass becomes part of the real prerequisite set.
- The primitive name stays the same, but the solving assumptions change materially at 2.32.
