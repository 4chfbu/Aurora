...
- Family: `Safe-Linking And Modern Tcache`
- README summary: Arbitrary decimal value and chunk pointer writing in heap by out-of-bounds tcache metadata writing
- Core effect: relative write into heap and tcache metadata, including controlled numeric writes and chunk-pointer writes.
- README applicability: `>= 2.30`
- Solver window note: `README marks `>= 2.30`; local examples are present from `2.31 -> 2.41`

## When To Consider It

- bounded out-of-bounds writes near tcache metadata.

## Version Groups

- README marks `>= 2.30`; local examples are present from `2.31 -> 2.41`.

## Version Differences

- This is already a modern technique family, so all retained versions are post-tcache and near or inside safe-linking era assumptions.
- The main differences across retained versions are allocator-environment adjustments rather than a different core idea.
- Use it when the corruption is relative and local to tcache metadata rather than a fully arbitrary pointer write.
