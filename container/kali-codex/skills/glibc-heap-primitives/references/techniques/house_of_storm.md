...
- README summary: Exploiting a use after free on both a large and unsorted bin chunk to return an arbitrary chunk from malloc
- Core effect: arbitrary chunk return using UAF on largebin and unsorted-bin chunks.
- README applicability: `< 2.29`
- Solver window note: `2.23 -> 2.27`

## When To Consider It

- historical mixed-bin UAF with both bins in play.
...
- 2.23 -> 2.24 is the original mixed unsorted-bin and largebin UAF chain.
- 2.27 retargets the technique to the short post-tcache historical window, documents stronger prerequisites, and explicitly assumes tcache must be saturated before the old bin behavior is visible.
- The later historical variant also retunes heap layout details such as filler sizes and setup bookkeeping to keep the unsorted-bin attack path viable.
