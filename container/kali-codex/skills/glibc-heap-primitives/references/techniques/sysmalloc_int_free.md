...
- README summary: Demonstrating freeing the nearly arbitrary sized Top Chunk (Wilderness) using malloc (sysmalloc  `_int_free()` )
- Core effect: free or recycle an almost arbitrary-sized top chunk through allocator path confusion.
- README applicability: `latest`
- Solver window note: `2.23 -> 2.41`

## When To Consider It

- top-chunk behavior during large allocation growth.
...
- The core family stays about top-chunk interaction during large allocation growth.
- The retained variants mainly demonstrate that the idea survives across many allocator generations, not that every runtime behaves identically without adjustment.
- Treat the target allocator path as the real version-specific question.
