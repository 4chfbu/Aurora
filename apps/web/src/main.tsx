...
type RuntimePolicy = { subagents_enabled: boolean; max_subagents_per_worker: number; max_subagents_concurrent: number; multi_agent_exploration_enabled: boolean; max_parallel_explorers: number; max_reason_intents: number; max_pending_intents: number };
...
type AgentRuntimeSettings = { multi_agent_exploration_enabled: boolean; max_global_workers: number; default_max_project_workers: number; default_max_reason_intents: number; default_max_pending_intents: number; subagents_enabled: boolean; default_max_subagents_per_worker: number; default_max_subagents_concurrent: number; max_challenge_group_concurrent: number; updated_at?: string };
...
type OpenVPNConfig = { configured: boolean; locked: boolean; connected: boolean; desired_connected: boolean; state: 'unconfigured' | 'disconnected' | 'connected' | 'error'; routes: string[]; credentials_configured: boolean; reconfigure_required?: boolean; last_error?: string | null; updated_at?: string | null };
...
function App() {
  ...
  const [agentSettings, setAgentSettings] = useState<AgentRuntimeSettings>({ multi_agent_exploration_enabled: false, max_global_workers: 4, default_max_project_workers: 2, default_max_reason_intents: 3, default_max_pending_intents: 8, subagents_enabled: false, default_max_subagents_per_worker: 2, default_max_subagents_concurrent: 2, max_challenge_group_concurrent: 2 });
  ...
  async function loadAgentSettings() { try { const saved = await request<AgentRuntimeSettings>('/api/settings/agents'); setAgentSettings(saved); setMultiAgentExploration(saved.multi_agent_exploration_enabled); setMaxParallelExplorers(saved.default_max_project_workers); } catch { /* Older API instances may require a restart after this frontend update. */ } }
  async function saveAgentSettings(config: AgentRuntimeSettings) { await guarded(async () => { const saved = await request<AgentRuntimeSettings>('/api/settings/agents', { method: 'PUT', body: JSON.stringify(config) }); setAgentSettings(saved); setMultiAgentExploration(saved.multi_agent_exploration_enabled); setMaxParallelExplorers(saved.default_max_project_workers); }); }
  async function saveProjectRuntimePolicy(policy: RuntimePolicy) { if (!selectedProjectId) return; await guarded(async () => { const saved = await request<RuntimePolicy>(`/api/projects/${selectedProjectId}/runtime-policy`, { method: 'PUT', body: JSON.stringify(policy) }); setBlackboard((current) => current ? { ...current, runtime_policy: saved } : current); }); }
  async function saveGroupRuntimePolicy(maxConcurrent: number) { if (!selectedGroupId) return; await guarded(async () => { const saved = await request<ChallengeGroup>(`/api/challenge-groups/${selectedGroupId}/runtime-policy`, { method: 'PUT', body: JSON.stringify({ max_concurrent: maxConcurrent }) }); setGroupDetail((current) => current ? { ...current, group: saved } : current); setGroups((current) => current.map((group) => group.id === saved.id ? saved : group)); }); }
  ...
  async function saveOpenVPN(input: { file: File; routes: string; username: string; password: string }) { await guarded(async () => { const form = new FormData(); form.set('ovpn', input.file); form.set('routes', input.routes); if (input.username) form.set('username', input.username); if (input.password) form.set('password', input.password); setOpenVPN(await request<OpenVPNConfig>('/api/settings/openvpn', { method: 'PUT', body: form })); }); }
  async function openvpnCommand(action: 'connect' | 'disconnect') { await guarded(async () => { setOpenVPN(await request<OpenVPNConfig>(`/api/settings/openvpn/${action}`, { method: 'POST' })); }); }
  ...
  <main className="ops-shell">
    <aside className="command-rail">
      ...
      <nav className="command-actions" aria-label="项目控制">
        ...
        <IconAction icon={<Bot />} label="Agent 分配" disabled={busy} onClick={() => { setMessage(''); setShowAgentSettings(true); }} />
  ...
  {showAgentSettings ? <AgentSettingsDialog busy={busy} globalSettings={agentSettings} projectPolicy={blackboard?.runtime_policy ?? null} group={groupDetail?.group ?? null} onSaveGlobal={(config) => void saveAgentSettings(config)} onSaveProject={(policy) => void saveProjectRuntimePolicy(policy)} onSaveGroup={(maxConcurrent) => void saveGroupRuntimePolicy(maxConcurrent)} onClose={() => setShowAgentSettings(false)} /> : null}
  ...
  {showOpenVPN ? <OpenVPNDialog busy={busy} config={openvpn} error={message} onSave={(input) => void saveOpenVPN(input)} onCommand={(action) => void openvpnCommand(action)} onClear={() => void clearOpenVPN()} onClose={() => setShowOpenVPN(false)} /> : null}
...
function AgentSettingsDialog({ busy, globalSettings, projectPolicy, group, onSaveGlobal, onSaveProject, onSaveGroup, onClose }: { busy: boolean; globalSettings: AgentRuntimeSettings; projectPolicy: RuntimePolicy | null; group: ChallengeGroup | null; onSaveGlobal: (config: AgentRuntimeSettings) => void; onSaveProject: (policy: RuntimePolicy) => void; onSaveGroup: (maxConcurrent: number) => void; onClose: () => void }) {
  const [globalDraft, setGlobalDraft] = useState(globalSettings);
  const [projectDraft, setProjectDraft] = useState<RuntimePolicy | null>(projectPolicy);
  const [groupMax, setGroupMax] = useState(group?.max_concurrent ?? 1);
  useEffect(() => { setGlobalDraft(globalSettings); }, [globalSettings.updated_at]);
  useEffect(() => { setProjectDraft(projectPolicy); }, [projectPolicy?.subagents_enabled, projectPolicy?.max_subagents_per_worker, projectPolicy?.max_subagents_concurrent, projectPolicy?.multi_agent_exploration_enabled, projectPolicy?.max_parallel_explorers, projectPolicy?.max_reason_intents, projectPolicy?.max_pending_intents]);
  useEffect(() => { setGroupMax(group?.max_concurrent ?? 1); }, [group?.id, group?.max_concurrent]);
  const globalValid = globalDraft.default_max_project_workers <= globalDraft.max_global_workers && globalDraft.default_max_subagents_concurrent <= globalDraft.default_max_subagents_per_worker;
  const projectValid = Boolean(projectDraft && projectDraft.max_subagents_concurrent <= projectDraft.max_subagents_per_worker);
  const globalNumber = (name: keyof AgentRuntimeSettings, value: string) => setGlobalDraft({ ...globalDraft, [name]: Number(value) });
  const projectNumber = (name: keyof RuntimePolicy, value: string) => projectDraft && setProjectDraft({ ...projectDraft, [name]: Number(value) });
  return <div className="import-backdrop" role="dialog" aria-modal="true" aria-label="Agent 分配设置"><section className="import-dialog agent-settings-dialog"><header><div><span className="eyebrow">LIVE SCHEDULER CONTROL</span><h2>Agent 分配</h2></div><button className="icon-button" title="关闭" aria-label="关闭" onClick={onClose}><X size={17} /></button></header><p>保存后从下一次派发生效；已运行的 Worker 不会被强制终止。</p><section className="agent-settings-section"><div className="agent-settings-heading"><div><strong>全局上限与新项目默认值</strong><small>跨项目公平分配和默认策略</small></div><button className="primary-button" disabled={busy || !globalValid} onClick={() => onSaveGlobal(globalDraft)}><Check size={14} />保存全局</button></div><div className="agent-toggle-grid"><label><input type="checkbox" checked={globalDraft.multi_agent_exploration_enabled} onChange={(event) => setGlobalDraft({ ...globalDraft, multi_agent_exploration_enabled: event.target.checked })} />启用多 Agent 探索</label><label><input type="checkbox" checked={globalDraft.subagents_enabled} onChange={(event) => setGlobalDraft({ ...globalDraft, subagents_enabled: event.target.checked })} />启用容器内 Subagent</label></div><div className="agent-number-grid"><label>全局 Explore Worker<input type="number" min={1} max={32} value={globalDraft.max_global_workers} onChange={(event) => globalNumber('max_global_workers', event.target.value)} /></label><label>默认项目 Explore<input type="number" min={1} max={32} value={globalDraft.default_max_project_workers} onChange={(event) => globalNumber('default_max_project_workers', event.target.value)} /></label><label>默认 Reason Intent<input type="number" min={1} max={32} value={globalDraft.default_max_reason_intents} onChange={(event) => globalNumber('default_max_reason_intents', event.target.value)} /></label><label>默认 Pending Intent<input type="number" min={1} max={128} value={globalDraft.default_max_pending_intents} onChange={(event) => globalNumber('default_max_pending_intents', event.target.value)} /></label><label>默认每 Worker Subagent<input type="number" min={1} max={16} value={globalDraft.default_max_subagents_per_worker} onChange={(event) => globalNumber('default_max_subagents_per_worker', event.target.value)} /></label><label>默认并行 Subagent<input type="number" min={1} max={16} value={globalDraft.default_max_subagents_concurrent} onChange={(event) => globalNumber('default_max_subagents_concurrent', event.target.value)} /></label><label>题组全局并发上限<input type="number" min={1} max={32} value={globalDraft.max_challenge_group_concurrent} onChange={(event) => globalNumber('max_challenge_group_concurrent', event.target.value)} /></label></div></section><section className="agent-settings-section"><div className="agent-settings-heading"><div><strong>当前项目</strong><small>{projectPolicy ? '独立于新项目默认值' : '未选择项目'}</small></div><button className="primary-button" disabled={busy || !projectValid || !projectDraft} onClick={() => projectDraft && onSaveProject(projectDraft)}><Check size={14} />保存项目</button></div>{projectDraft ? <><div className="agent-toggle-grid"><label><input type="checkbox" checked={projectDraft.multi_agent_exploration_enabled} onChange={(event) => setProjectDraft({ ...projectDraft, multi_agent_exploration_enabled: event.target.checked })} />多 Agent 探索</label><label><input type="checkbox" checked={projectDraft.subagents_enabled} onChange={(event) => setProjectDraft({ ...projectDraft, subagents_enabled: event.target.checked })} />容器内 Subagent</label></div><div className="agent-number-grid"><label>并行 Explore<input type="number" min={1} max={32} value={projectDraft.max_parallel_explorers} onChange={(event) => projectNumber('max_parallel_explorers', event.target.value)} /></label><label>每轮 Reason Intent<input type="number" min={1} max={32} value={projectDraft.max_reason_intents} onChange={(event) => projectNumber('max_reason_intents', event.target.value)} /></label><label>Pending Intent 上限<input type="number" min={1} max={128} value={projectDraft.max_pending_intents} onChange={(event) => projectNumber('max_pending_intents', event.target.value)} /></label><label>每 Worker Subagent<input type="number" min={1} max={16} value={projectDraft.max_subagents_per_worker} onChange={(event) => projectNumber('max_subagents_per_worker', event.target.value)} /></label><label>并行 Subagent<input type="number" min={1} max={16} value={projectDraft.max_subagents_concurrent} onChange={(event) => projectNumber('max_subagents_concurrent', event.target.value)} /></label></div></> : <p className="muted">选择项目后可修改其 Agent 策略。</p>}</section><section className="agent-settings-section"><div className="agent-settings-heading"><div><strong>当前题组</strong><small>{group?.name ?? '未选择题组'}</small></div><button className="primary-button" disabled={busy || !group || groupMax < 1 || groupMax > 32} onClick={() => onSaveGroup(groupMax)}><Check size={14} />保存题组</button></div><div className="agent-number-grid"><label>题组并行项目<input type="number" min={1} max={32} disabled={!group} value={groupMax} onChange={(event) => setGroupMax(Number(event.target.value))} /></label></div></section></section></div>;
}
...
function OpenVPNDialog({ busy, config, error, onSave, onCommand, onClear, onClose }: { busy: boolean; config: OpenVPNConfig; error: string; onSave: (input: { file: File; routes: string; username: string; password: string }) => void; onCommand: (action: 'connect' | 'disconnect') => void; onClear: () => void; onClose: () => void }) {
  const [file, setFile] = useState<File | null>(null);
  const [routes, setRoutes] = useState(config.routes.join('\n'));
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  useEffect(() => { setRoutes(config.routes.join('\n')); }, [config.routes.join('|')]);
  const canSave = Boolean(file && routes.trim() && Boolean(username) === Boolean(password));
  const stateLabel = { unconfigured: '未配置', disconnected: '未连接', connected: '已连接', error: config.reconfigure_required ? '需要重新上传' : '连接异常' }[config.state];
  return <div className="import-backdrop" role="dialog" aria-modal="true" aria-label="OpenVPN 设置"><section className="import-dialog openvpn-dialog"><header><div><span className="eyebrow">ISOLATED VPN GATEWAY</span><h2>OpenVPN</h2></div><button className="icon-button" title="关闭" aria-label="关闭" onClick={onClose}><X size={17} /></button></header><div className={`openvpn-state ${config.state}`}><strong>{stateLabel}</strong><span>{config.connected ? 'Solver Worker 将通过隔离隧道访问指定网段' : '未连接时 Worker 保持原有网络路径'}</span></div>{error ? <div className="notice error">{error}</div> : null}{config.last_error ? <div className="openvpn-error">{config.last_error}</div> : null}<label>OVPN 文件<input type="file" accept=".ovpn,text/plain" onChange={(event) => setFile(event.target.files?.[0] || null)} /></label><label>VPN 转发网段<textarea value={routes} onChange={(event) => setRoutes(event.target.value)} placeholder={'10.10.0.0/16\n172.20.1.8'} spellCheck={false} /></label><div className="openvpn-grid"><label>VPN 用户名（可选）<input value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="off" /></label><label>VPN 密码（可选）<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" /></label></div>{config.routes.length ? <div className="openvpn-routes"><span>当前隧道路由</span><code>{config.routes.join(' · ')}</code></div> : null}<footer><span>本机密钥加密 · ROUTE-NOPULL · FAIL CLOSED</span><div>{config.configured && !config.reconfigure_required && !config.connected ? <button className="primary-button" disabled={busy} onClick={() => onCommand('connect')}><Play size={15} />连接</button> : null}{config.connected || config.desired_connected ? <button className="danger-button" disabled={busy} onClick={() => onCommand('disconnect')}><PauseCircle size={15} />断开</button> : null}<button className="primary-button" disabled={busy || !canSave} onClick={() => file && onSave({ file, routes, username, password })}><Check size={15} />保存配置</button>{config.configured ? <button className="danger-button" disabled={busy || config.connected} onClick={onClear}><Trash2 size={15} />清除</button> : null}</div></footer></section></div>;
}
